...
void do_block_512_omp(double * restrict A, double * restrict B, double * restrict C, const int n,
...
void dgemm_avx512_unroll_block_omp(double * restrict A, double * restrict B, double * restrict C, const int n) {
	#pragma omp parallel for
	for (int si = 0; si < n; si += BLOCKSIZE) {
		for (int sj = 0; sj < n; sj += BLOCKSIZE) {
			for (int sk = 0; sk < n; sk += BLOCKSIZE) {
				do_block_512_omp(A, B, C, n, si, sj, sk);
			}
		}
	}
}